...
int main(void)
{
	...
	const char *arr_one_attention[] = {"#0P1220T4000\r\n", "#2P1500T4000\r\n", "#1P1550T4000\r\n", "#5P1580T4000\r\n", "#4P1300T4000\r\n", "#3P2000T4000\r\n"};
...
*/
	...
	while(1){
		...
		case 1:	//发送Attention指令
			if((write(fd_serial, arr_one_attention[x], strlen(arr_one_attention[x]))) == -1){
				printf("Write arr_one_attention[%d]:\"%s\" error.\n", x, arr_one_attention[x]);
				close(fd_serial);
				exit(1);
			}
			usleep(get_time(arr_one_attention[x]));	//延时时间为该条指令完成所需时间
			printf("OK.\n");
			x++;
			if(x == 6){
				close(fd_serial);
