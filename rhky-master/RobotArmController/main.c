...
int main(void)
{
	...
	const char *arr_right_hands_up[] = {"#0P2080#2P1500#1P1550T3000\r\n", "#5P1580#4P1300#3P2000T3000\r\n"};
	const char *arr_left_hands_up[] = {"#0P1220#2P1500#1P1550T3000\r\n", "#5P1580#4P1300#3P920T3000\r\n"};
...
*/
	...
	while(1){
		...
		case 1:	//发送Attention指令
			...
			printf("OK.\n");
			break;
		case 2:	//发送Attention指令
			for(x = 0; x < sizeof(arr_right_hands_up) / sizeof(char*); x++){	//每次发送一条指令
				if((write(fd_serial, arr_right_hands_up[x], strlen(arr_right_hands_up[x]))) == -1){
					printf("Write arr_right_hands_up[%d]:\"%s\" error.\n", x, arr_right_hands_up[x]);
					close(fd_serial);
					exit(1);
				}
				usleep(get_time(arr_right_hands_up[x]));	//延时时间为该条指令完成所需时间
			}
			printf("OK.\n");
			break;
		case 3:	//发送Attention指令
			for(x = 0; x < sizeof(arr_left_hands_up) / sizeof(char*); x++){	//每次发送一条指令
				if((write(fd_serial, arr_left_hands_up[x], strlen(arr_left_hands_up[x]))) == -1){
					printf("Write arr_left_hands_up[%d]:\"%s\" error.\n", x, arr_left_hands_up[x]);
					close(fd_serial);
					exit(1);
				}
				usleep(get_time(arr_left_hands_up[x]));	//延时时间为该条指令完成所需时间
			}
			printf("OK.\n");
			break;
